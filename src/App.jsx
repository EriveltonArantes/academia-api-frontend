    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Plano</li>
...
function PainelPlano({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "descricao", "valor", "duracaoMeses", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/planos", { headers: authHeaders(token) })
  ...
  React.useEffect(() => {
    ...
    fetch(apiBaseUrl + "/api/planos", { headers: authHeaders(token) }).then(r => r.json()).then(setPlanoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/planos/" + editId : apiBaseUrl + "/api/planos";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "descricao", "valor", "duracaoMeses"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno", "plano"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/planos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Plano")}</div>
            ...
            <div className="item-field"><b>duracaoMeses</b><span>{item["duracaoMeses"] ?? "—"}</span></div>
            <div className="item-field"><b>aluno</b><span>{item.alunoId ? ((alunoList.find(o => o.id === item.alunoId) || {}).nome ?? ("#" + item.alunoId)) : "—"}</span></div>
            <div className="item-field"><b>plano</b><span>{item.planoId ? ((planoList.find(o => o.id === item.planoId) || {}).nome ?? ("#" + item.planoId)) : "—"}</span></div>
            </div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Plano</h3>
      ...
      <label className="field-label">duracaoMeses</label>
      <input type="number" value={form["duracaoMeses"] ?? ""} onChange={e => setForm({...form, duracaoMeses: e.target.value})} />
      ...
      <select value={form["plano"] ?? ""} onChange={e => setForm({...form, plano: e.target.value})}>
        <option value="">Selecione...</option>
        {(planoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
...
function PainelAluno({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "dataNascimento", "telefone", "email", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "dataNascimento", "telefone", "email"].forEach(k => { f[k] = item[k] ?? ""; });
...
function PainelMatricula({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoId", "planoId", "dataInicio", "dataVencimento", "status", "valorPago", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["alunoId", "planoId", "dataInicio", "dataVencimento", "status", "valorPago"].forEach(k => { f[k] = item[k] ?? ""; });
    ["aluno", "plano"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>planoId</b><span>{item["planoId"] ?? "—"}</span></div>
            ...
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.valorPago)}>Cobrar via Pix</button>
      ...
      <label className="field-label">planoId</label>
      <input type="number" value={form["planoId"] ?? ""} onChange={e => setForm({...form, planoId: e.target.value})} />
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Plano");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Academia<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Plano" ? " active" : "")} onClick={() => setAba("Plano")}><span className="nav-ico">🗂️</span>Plano</button>
      ...
      <main className="main">
        ...
        {aba === "Plano" && <PainelPlano token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
